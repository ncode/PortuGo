...
func TestExpressionGolden(t *testing.T) {
	sourcePath := "../../testdata/format/expression_forms.alg"
	wantPath := "../../testdata/format/expression_forms.formatted.alg"
	...
	if !bytes.Equal(formatted.Bytes(), want) {
		t.Fatalf("formatted expression golden mismatch:\ngot:\n%s\nwant:\n%s", formatted.Bytes(), want)
	}
	_, reparsedTokens, lexDiags := lexer.Scan("expression_forms.alg", formatted.String())
	if len(lexDiags) != 0 {
	...
	if !bytes.Equal(reformatted.Bytes(), formatted.Bytes()) {
		t.Fatalf("expression formatting is not idempotent:\nfirst:\n%s\nsecond:\n%s", formatted.Bytes(), reformatted.Bytes())
	}
}
