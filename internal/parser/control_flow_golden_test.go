...
func TestControlFlowGolden(t *testing.T) {
	sourcePath := "../../testdata/format/control_flow_forms.alg"
	wantPath := "../../testdata/format/control_flow_forms.formatted.alg"
	...
	if !bytes.Equal(formatted.Bytes(), want) {
		t.Fatalf("formatted control-flow golden mismatch:\ngot:\n%s\nwant:\n%s", formatted.Bytes(), want)
	}
	_, reparsedTokens, lexDiags := lexer.Scan("control_flow_forms.alg", formatted.String())
	if len(lexDiags) != 0 {
