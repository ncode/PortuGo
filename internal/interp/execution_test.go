...
func TestExecutionDiagnostics(t *testing.T) {
	for _, tt := range []struct {
		...
		{"substring position conversion", "", "escreval(copia(\"abc\",1e20,2))", "copia", diag.RBuiltin, Options{}},
		{"substring length conversion", "", "escreval(copia(\"abc\",1,1e20))", "copia", diag.RBuiltin, Options{}},
		{"real multiplication overflow", "", "escreval((10 ^ 200) * (10 ^ 200))", "*", diag.RArithmetic, Options{}},
		{"real addition overflow", "", "escreval((10 ^ 308) + (10 ^ 308))", "+", diag.RArithmetic, Options{}},
		{"real subtraction overflow", "", "escreval((10 ^ 308) - (-(10 ^ 308)))", "-", diag.RArithmetic, Options{}},
		{"real division overflow", "", "escreval((10 ^ 308) / (10 ^ (-308)))", "/", diag.RArithmetic, Options{}},
...
func TestOptionsRandomAndDefaults(t *testing.T) {
	p, info := analyzed(t, "algoritmo \"random\"\ninicio\nescreval(rand, randi(4), randi(3))\nescreval(RaNd)\nfimalgoritmo")
	r := &scriptedRandom{}
	var out bytes.Buffer
	i := New(Options{Random: r, Output: &out})
	if ds := i.Run(p, info); len(ds) != 0 || out.String() != " 0.25 3 2\n 0.25\n" || r.fractions != 2 {
