...
- [x] 2.2 Inventory every `[VERIFICAR]`, original compatibility-checklist item, audited defect, current behavior assumption, environment command, official feature, and bundled `.alg` example into stable manifest IDs.
...
- [x] 2.5 Implement and test evidence, incremental, and implementation-acceptance validation modes, including hashes, path containment, prohibited artifacts, inventory coverage, stale links, owner-group readiness, and forbidden unreviewed verified-to-pending downgrades.
  ...
  modes. Test links now require Go's test-name and declaration shape, rejecting
  helpers, methods, harness-only entry points and invalid signatures while
  preserving valid imported parameter forms. All three modes have regression
  coverage; package compilation and execution remain separate checks.
  A coverage downgrade also requires review metadata distinct from the earlier
  qualification review. Reusing that review, including a whitespace-only
  rationale edit, fails in every mode; a reviewed downgrade remains pending
  at the implementation-acceptance gate.
  Reviewed exclusions also retain hash checks for screenshots, transcriptions
  and generated bytes, with contained, unique generated paths. Generic
  regressions cover these retained observations in all three modes.
  Optional candidate stdout also receives artifact path and hash checks on
  exclusions. Generic regressions reject altered, missing and malformed
  artifacts while preserving omitted output and valid independent bytes.
  Retained reference file layouts now receive the same hierarchy checks against
  initial fixtures, independently of candidate outputs. Exclusions still need
  no generated-output counterpart; valid separate layouts remain covered.
  ...
  Stdout, state and host observation artifacts also share the adapter's 1 MiB
  output limit, so oversized retained expectations fail before replay.
  ...
  command family through existing probe coverage, including random input, file
  input, and console configuration. These links do not
  ...
  the remaining non-numbered markers remain open.
  The scientific-notation verification marker now links to its recorded probe;
  the remaining non-numbered markers remain open.
  The string-quote escape verification marker now links to its recorded
  rejection probes; the remaining non-numbered markers remain open.
  The accented-comparison verification marker now links to its recorded probe;
  the remaining non-numbered markers remain open.
  The brace-comment verification marker now links to its recorded probe; the
  remaining non-numbered markers remain open.
  The choice-range verification marker now links to its recorded probe; the
  remaining non-numbered markers remain open.
  The integer-truncation verification marker now links to its recorded probe;
  the remaining non-numbered markers remain open.
  The accented-uppercase verification marker now links to its recorded probe;
  the remaining non-numbered markers remain open.
  The case-sensitive string-position verification marker now links to its
  recorded probe; the remaining non-numbered markers remain open.
  The exact-integer-division verification marker now links to its recorded
  probe; the remaining non-numbered markers remain open.
  The numeric-to-text real-format verification marker now links to its recorded
  probe; the remaining non-numbered markers remain open.
  The invalid numeric-conversion verification marker now links to its recorded
  probe; the remaining non-numbered markers remain open.
  The default-real-output verification marker now links to its recorded probe;
  the remaining non-numbered markers remain open.
  The output-width spacing verification marker now links to its recorded
  probe; the remaining non-numbered markers remain open.
  The typed-input validation verification marker now links to its recorded
  probe; the remaining non-numbered markers remain open.
  The for-loop final-value verification marker now links to its recorded
  probe; the remaining non-numbered markers remain open.
  The for-loop control-mutation verification marker now links to its recorded
  probe; the remaining non-numbered markers remain open.
  The procedure bare-return verification marker now links to its recorded
  probe; the remaining non-numbered markers remain open.
  The function-without-return verification marker now links to its recorded
  probe; the remaining non-numbered markers remain open.
  The main-program return verification marker now links to its recorded probe;
  the remaining non-numbered markers remain open.
  The vector-element reference verification marker now links to its recorded
  probe; the remaining non-numbered markers remain open.
  The ranged-random domain and precision marker now links to its recorded
  probes; statistical uniformity and the remaining non-numbered markers remain
  open.
  The generated file-format marker now links to its recorded probe; separate
  ...
  are recognized by trace validation without inventing probe links. At that point,
  this slice did not complete task 2.2 because other source questions remained open.
  At that point, this slice did not complete tasks 2.2 or 17.2.
  ...
  recordings add stable links for reserved words, declaration sections,
  outside-loop interruption, builtin collisions, reference-argument
  assignability, and finite recursion depth. The nonassignable-argument
  diagnostic contract, maximum recursion or stack-display behavior, and the
  remaining source questions remain open; that slice did not promote them or
  complete tasks 2.2 or 17.2.

  A final inventory audit now resolves 229 stable records: 73 requirements,
  73 bundled examples, 25 checklist rows, 48 assumptions, 9 feature records,
  and one audited defect. All 33 legacy `[VERIFICAR]` markers and 1,679 probes
  have bidirectional trace links, including the environment-command records.
  Task 2.2 is complete; the two unsupported `var`-argument and recursion/stack
  questions retain their pending classifications under their owning tasks.
...
- [x] 2.8 Create reduced lexical, newline, identifier, keyword, comment, literal, declaration-order, post-`fimalgoritmo`, recovery, and canonical-syntax probe batches.

  The reduced lexical and grammar batch now spans nine requirement traces and
  152 unique implementation-verified probes, including 149 recorded cases and
  three reviewed project-only contracts. Broader declaration ordering,
  recovery, canonical-syntax, and other unqualified forms remain pending under
  their implementation tasks.
- [x] 2.9 Create reduced type, constant, alias, record, field, vector, 500-slot, copying, reference-parameter, call-form, visibility, recursion, and return probe batches, recording positive support and rejection separately.

  The reduced type and declaration batch now spans fourteen requirement traces
  and 273 unique recorded and implementation-verified probes. Fresh recordings
  cover empty constant sections, constant bounds, record and vector copying,
  parameterless calls, one reference coercion, zero/positive/negative bounds,
  and accepted vector sizes through 5001. Upper storage boundaries, complete
  aliasing rules, visibility, returns, and the reviewed nonassignable-argument
  faults remain pending under their owning tasks.
...
- [x] 2.11 Create reduced console/CP1252/arquivo/random/environment probe batches and a built-in inventory covering every name in the standard-library spec, including the seven missing numeric functions and the two-argument `exp` candidate.
  ...
  configuration families to their existing verified probes, alongside the
...
- [x] 17.2 Complete bidirectional trace links for every OpenSpec requirement, `[VERIFICAR]`, original checklist item, audited defect, discovered official feature, implementation test, and bundled example with no stale IDs.
  ...
  Shared probe links remain intentional; the 12 pending implementation probes
...
- [ ] 17.3 Run every accepted official VisuAlg 3.0.7 bundled example and eliminate all deterministic output, error, state, and generated-file mismatches; record reviewed reasons for every non-accepted example.
  ...
  and after formatting; eight additional accepted programs now have reviewed
  random-output contracts. Seven generated-output cases still need a
  ...
  Auditing the eight remaining pending accepted programs found successful
  ...
  contract; the mixed integer/text, integer-sort, real-sort, record-sort, and
  integer-search transcripts are now qualified as well, leaving seven
  generated-output cases and one frontend rejection.
  The parser now
...
- [x] 17.4 Exercise the assembled source/depth/call/value/step guards, including empty infinite loops, recursive calls, flat AST chains, input retries, and cleanup, plus storage/encoding/host/filesystem adversarial cases; require controlled diagnostics and fail on a subprocess watchdog kill or accepted-example budget exhaustion.
  ...
  The assembled safeguard sweep is recorded in
  `docs/safeguard-acceptance.md` and linked as `project.assembled-safeguards`.
  Source and parser bounds, runtime recursion/step/evaluation limits, flat
  operand retention, input and file cleanup, storage validation, CP1252
  decoding, host restoration, and REPL recovery all have focused tests. The
...
- [x] 17.8 Perform a clean-checkout release rehearsal of all CLI commands, REPL transcripts, fixtures, manifest validation, and build artifacts on the supported platforms.
  ...
  acceptance stays blocked by the 12 pending implementation probes and final
  handoff tasks.
