...
- Complete the oracle-confirmed language surface, including physical-newline rules, declarations, vectors, subprogram call variants, returns, expressions, and control flow. Group 2 recordings establish positive support only for the scoped constant, scalar-alias, named-record, scalar-field, assignment-alias, and case-range forms documented by the delta specifications; rejected variants retain negative compatibility coverage, and unrecorded combinations remain pending without dependent AST or example obligations.
