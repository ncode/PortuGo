...
See `proposal.md` for motivation and the eight delta specifications for observable requirements. The current repository is a small Go 1.22 tree walker whose semantic entry point returns diagnostics only, whose interpreter reparses AST type syntax and returns an unpositioned `error`, whose built-in signatures and implementations are duplicated, and whose CLI and REPL construct input streams independently. The AST separates globals from subprograms, stores vector bounds as integers, and loses comments during lexing. Declaration ordering, bound expressions, optional forms, and environment commands need evidence before their syntax is expanded. Group 2 recordings now establish scoped positive support for constants, scalar aliases, named records, scalar fields, assignment aliases, and case ranges; unsupported variants remain negative coverage and unrecorded forms remain evidence-gated.
...
Before group 3, group 2 corrects `proposal.md`, the delta specs, `design.md`, **and `tasks.md`**, along with affected project documentation, using the recorded dispositions. The current corpus gives positive requirements only to the scoped constant, scalar-alias, named-record, scalar-field, assignment-alias, and case-range forms it records. Rejected variants use positioned negative coverage; unrecorded forms remain pending, with no dependent layouts or examples required. The original probe and requirement/task ID disposition remains in the manifest, and retired IDs retain an explicit replacement or reviewed disposition. Groups 3, 5–9, and 16 consume that revised scope; they must not construct unused record/type machinery just to satisfy the original plan.
...
The fuzz/adversarial profile uses at most 64 KiB of generated source, the structural limits above, and 10,000 runtime steps. Tests inject finite input, output capture capped at 1 MiB (returning a writer error on overflow), scripted randomness, and a nonblocking fake host/clock. File probes use isolated temporary directories. Potentially blocking or crashing cases in this profile run in child processes with a five-second watchdog. Finite full-source-boundary cases that decode or format 4 MiB may use an explicit 30-second watchdog to allow instrumentation under load; oversized-source rejection and depth cases retain five seconds. A watchdog kill is a failing test, never a successful language diagnostic, even if the child printed `PASS` before exiting. Ordinary interactive reads may wait for the user. Deterministic corpus entries record sufficient per-probe budgets; budget exhaustion or timeout in an accepted example fails acceptance. No goroutines are added to the interpreter to enforce these limits.
