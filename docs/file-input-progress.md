...
13 positioned rejections. All 51 have verified implementation coverage. The two
...
The replay manifest marks the existing fixture in the six constrained controls
as unavailable during the child process. POSIX replay temporarily removes its
read permissions; Windows replay holds a read/write handle with sharing
disabled. Both setups make the candidate open return an unavailable-file error,
then restore access before generated bytes are compared. Platform regressions
still classify the native permission-denied and sharing-failure errors, while
the shared fallback behavior is checked by the replayed controls.
...
have an automated candidate ACL fixture. The three `file-access-denied-*` and
three `file-access-locked-*` probes use the same unavailable-open replay
contract because the observable continuation is shared. The native focused
regression remains responsible for checking the Windows sharing lock, and error
mapping tests cover both native failure classes. No reference restriction is
