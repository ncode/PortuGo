...
`file-access-locked-*` probes also remain pending because the corpus runner
cannot recreate the lock; a native Windows run of the focused regression
exercises them and matches their recorded output and unchanged bytes, but does
not replace required replay. No interpreter file-open injection or
fixture-access schema is added.
