...
do not establish implementation acceptance: 12 implementation probes remain
...
| --- | ---: | --- |
| Bundled example qualification (17.3) | 12 | See the manifest for the current pending probe inventory. |
...
Eight bundled transcripts now have verified generated-output contracts. One
...
rows and a fixed not-found search. All replay the original and formatted source
without comparing random sequences. The remaining generated-output cases and
rejected paths stay pending.
